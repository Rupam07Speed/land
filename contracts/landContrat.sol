...
function updateLand(
    ...
    // Ensure the sender is the owner of this land
    require(lands[_landId].ownerAddress == msg.sender, "Only the land owner can update the land details.");

    // Update land details
    lands[_landId].area = _area;
    lands[_landId].landAddress = _address;
    lands[_landId].landPrice = _newPrice;
    lands[_landId].allLatitudeLongitude = _allLatiLongi; // Assuming the field in the struct is named 'allLatitudeLongitude'
    lands[_landId].propertyPID = _propertyPID;
    lands[_landId].physicalSurveyNumber = _surveyNum;
    lands[_landId].document = _document;

    // You can emit an event here to track changes if you want (optional)
